...
Adafruit_MQTT_Publish feed_sampling_rate_pub = Adafruit_MQTT_Publish(&mqtt, AIO_USERNAME "/feeds/sampling-rate");

Adafruit_MQTT_Subscribe feed_sampling_rate_sub = Adafruit_MQTT_Subscribe(&mqtt, AIO_USERNAME "/feeds/sampling-rate");
...
void setup() {
  ...
  mqtt.subscribe(&feed_sampling_rate_sub);
...
void loop() {
  ...
  while ((subscription = mqtt.readSubscription(5000))) {
    if (subscription == &feed_sampling_rate_sub) {
      Serial.print(F("*** Got: "));
      Serial.println((char *)feed_sampling_rate_sub.lastread);
      delay(100);
      new_time = true;
    }
  }
  ...
  if (new_time == true) {
    sampling_rate = atoi((char *)feed_sampling_rate_sub.lastread);
    delay(100);
  ...
  char sampleBuff[5];
  dtostrf(sampling_rate, 1, 2, sampleBuff);
  MQTT_publish_checkSuccess(feed_sampling_rate_pub, sampleBuff);
