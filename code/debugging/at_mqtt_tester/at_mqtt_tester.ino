...
Adafruit_MQTT_Publish feed_reply = Adafruit_MQTT_Publish(&mqtt, AIO_USERNAME "/feeds/reply");

// THE SUBSCRIBING FEEDS -----------------------------------------------------------------------------
Adafruit_MQTT_Subscribe feed_at = Adafruit_MQTT_Subscribe(&mqtt, AIO_USERNAME "/feeds/at-commands");

ATcommands module = ATcommands(FONA_RST, false);


void setup() {
  Serial.begin(9600);
  Serial.println(F("*** Executing at_mqtt_tester.ino ***"));
...
void loop() {
  ...
  while ((subscription = mqtt.readSubscription(5000))) {
    ...
    if (subscription == &feed_at) {
      Serial.print(F("*** Got: "));
      Serial.println((char *)feed_at.lastread);
      delay(100);
      strcpy(commandBuff, (char *)feed_at.lastread);
    }
  }
  
//  char replyBuff[25];
  fona.write(commandBuff);
  if(fona.available()) {
    Serial.write(fona.read());
    strcpy(replybuffer, fona.read());
  }
