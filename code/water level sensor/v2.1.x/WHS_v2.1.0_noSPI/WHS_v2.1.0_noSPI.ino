...
float prevDistance = -1;             // initialize values to impossible readings
...
//  }
  ...
  Serial.println("metrics: ---------------------------------------");
    Serial.print("Average distance: ");Serial.println(distance);
    
    // find first derivative
    if(prevDistance != -1) {
      float firstDeriv = abs(distance - prevDistance);
...
void changeTime(float avgSecondDeriv) {
  ...
  // equation to calculate time from derivatives (could be improved, it is very responsive)
