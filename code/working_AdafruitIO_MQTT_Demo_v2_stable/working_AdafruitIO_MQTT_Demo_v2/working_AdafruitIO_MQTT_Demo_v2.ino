...
char LEDString[3]; // Just for testing if needed!
...
Adafruit_MQTT_Publish feed_voltage = Adafruit_MQTT_Publish(&mqtt, AIO_USERNAME "/feeds/humidity");

// Setup a feed called 'command' for subscribing to changes.
Adafruit_MQTT_Subscribe feed_command = Adafruit_MQTT_Subscribe(&mqtt, AIO_USERNAME "/feeds/led-toggle");
Adafruit_MQTT_Subscribe feed_led = Adafruit_MQTT_Subscribe(&mqtt, AIO_USERNAME "/feeds/led-selection");
...
void setup() {
  Serial.begin(9600);
  Serial.println(F("*** SIMCom Module MQTT Example V2 ***"));
...
void loop() {
  ...
  // take pressure data
  float humidity = bmp.readPressure();
  ...
  */
  ...
  MQTT_publish_checkSuccess(feed_voltage, humBuff);



  int number = atoi((char *)feed_led.lastread);
  delay(100);
  Serial.print("Number: "); Serial.print((char *)feed_led.lastread); Serial.print(", "); Serial.println(number);
